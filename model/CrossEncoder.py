...
def prepare_data_cross_encoder(dataset, remove_col_names=True, order_cols=True, custom_dataset=False, blocked_pairs=None):
    table_a_serialized, table_b_serialized, X_train_ids, y_train, X_valid_ids, y_valid, X_test_ids, y_test = load_data(
        dataset, remove_col_names=remove_col_names, order_cols=order_cols, custom_dataset=custom_dataset)
    ...
    if blocked_pairs is not None:
        ...
        X_train_ids, X_test_ids, y_train, y_test = train_test_split(all_matches, [1 if x in all_true_matches else 0 for x in all_matches], test_size=0.2, random_state=42)
...
def fit_cross_encoder(model_name, train_loader, valid_set, y_valid, test_set, epochs=1, learning_rate=2e-5, weight_decay=0.01, classifier_dropout=None):
